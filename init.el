...
(use-package vulpea
  ...
  (defun js/roam-links--parse-existing (content level)
    "Parse existing dblock CONTENT into an alist of (id title . body).
LEVEL is the heading depth of entries."
  ...
  (defun js/roam-links--collect (notes level existing)
    "Insert sorted heading links for NOTES at heading depth LEVEL.
EXISTING is a list of (id title body) from the previous block contents."
    (let* ((prefix   (make-string level ?*))
           (live-ids (mapcar #'vulpea-note-id notes))
           (sorted   (seq-sort-by #'vulpea-note-title #'string< notes)))
      ;; Live nodes — use current title from DB, preserve body
      (dolist (note sorted)
	(let* ((id    (vulpea-note-id note))
          ...
          (when (and body (not (string-empty-p body)))
            (insert body "\n"))))
      ;; Dead nodes: no longer match query, but had content — preserve old title
...
Heading level is auto-detected from context if :level is not given."
    (let* ((tags         (plist-get params :tags))
           ...
           (existing     (js/roam-links--parse-existing content level))
           (notes
            (cond
           ...
           (notes
            (if exclude-tags
                (seq-filter (lambda (n)
                            ...
                            notes)
              notes)))
      (js/roam-links--collect notes level existing)))

  (defun js/roam-links--point-in-dblock-p ()
  ...
  (defun js/roam-links-maybe-autoupdate ()
    ...
    (when (and (string= "t" (cadr (car (org-collect-keywords
               ...
               (not (js/roam-links--point-in-dblock-p)))
      (save-excursion
