...
(string-to-number v) against a string, filtering out every row."
  (let* ((key (nth 0 meta-filter))
    ...
    (pcase op
      ...
      ((or '!= 'not= 'neq)
       `(lambda (n)
          (let ((v (vulpea-note-meta-get n ,key)))
            (not (or (equal v ,(if (numberp val) (number-to-string val) val))
                     ,(when (numberp val)
                        `(and v (= (string-to-number v) ,val))))))))
      ((or 'not-in 'not-member)
       (let* ((cands (if (listp val) val (list val)))
         ...
         `(lambda (n)
            (let ((vs (vulpea-note-meta-get-list n ,key)))
              (not (seq-some (lambda (v) (member v ',cand-strings)) vs))))))
      ((or 'in 'member)
       (let* ((cands (if (listp val) val (list val)))
              (cand-strings
               (mapcar (lambda (c)
