...
Values are read with the `link' TYPE, not the default `string':
`vulpea-buffer-meta-format' rewrites any string with a parseable URL
domain into \"[[url][domain]]\" on write, and a `string' read returns
that raw bracketed text -- so comparing against our plain computed
value would report every url-bearing note as changed on every run.
A `link' read returns the bare link path for bracketed values and
passes plain strings through, restoring exactly what we computed."
  (seq-some (lambda (kv)
              (not (equal (vulpea-note-meta-get-list note (car kv) 'link)
                          (cdr kv))))
            metadata))
...
`vulpea-utils-with-note' visits via `find-file-noselect' and leaves the
buffer alive; to avoid accumulating one buffer per stamped note, the
buffer is killed afterwards unless it was already open before the call."
  (let* ((path        (vulpea-note-path note))
         (preexisting (find-buffer-visiting path)))
